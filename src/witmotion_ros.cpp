...
bool ROSWitmotionSensorController::imu_recalibration = false;
...
ROSWitmotionSensorController::ROSWitmotionSensorController() : reader_thread(dynamic_cast<QObject*>(this)), node("~")
{
  ...
  node.param<bool>("imu_publisher/recalibrate", imu_recalibration, true);
  ...
  config_packet.setting.raw[0] = imu_recalibration ? 0x00 : 0x01;
  config_packet.setting.raw[1] = 0x01;
  ...
  if (temp_enable)
  {
    ...
    {
      ROS_WARN("Cannot determine message type to take temperature from (%s). Falling back to acceleration message",
               temp_from_str.c_str());
      temp_from = pidAcceleration;
    }
